...
export default function GameDetailClient({ game, initialInWishlist, initialInLibrary }: GameDetailClientProps) {
    if (!game) {
        return (
    ...
    // 3. Helper: Check if URL is video
    const isVideo = (url: string | undefined) => {
        if (!url) return false;
        return url.endsWith('.mp4') || url.endsWith('.webm') || url.endsWith('.ogg');
    };

    // Normalize gallery data (Handle Legacy Array vs New Object structure)
    const galleryList: string[] = (() => {
        if (!game.gallery) return [];

        // New Format: { images: string[], video: string | null }
        if (typeof game.gallery === 'object' && !Array.isArray(game.gallery)) {
            const images = Array.isArray(game.gallery.images) ? game.gallery.images : [];
            const video = game.gallery.video;
            return video ? [video, ...images] : images;
        }

        // Legacy Format: string[]
        return Array.isArray(game.gallery) ? game.gallery : [];
    })();

    // 4. Determine initial media
    const initialMedia = (() => {
        const firstVideo = galleryList.find((item: string) => isVideo(item));
        if (firstVideo) return firstVideo;
        // Fallback to hero image if no video/gallery
        return galleryList[0] || game.heroImage || game.image || game.imageUrl;
    })();

    // 5. State for active media
    // 5. State for active media
    const [activeMedia, setActiveMedia] = useState(initialMedia);

    // Use effective active media (state or derived default)
    const currentMedia = activeMedia || initialMedia;

    // --- State for wishlist and library ---
    ...
    const handleWishlistToggle = async () => {
        ...
        try {
            if (inWishlist) {
                const result = await removeFromWishlist(game.id);
                if (result.success) {
                    setInWishlist(false);
                }
            } else {
                const result = await addToWishlist(game.id);
                if (result.success) {
                    setInWishlist(true);
                } else if (result.error === 'Please log in to add to wishlist') {
                    router.push('/login');
                }
            }
        } catch (error) {
    ...
    // Handler for purchase - show payment modal
    ...
    const handleConfirmPayment = async () => {
        ...
        try {
            ...
            if (result.success) {
                setInLibrary(true);
                setInWishlist(false);
                alert(result.message || 'Game added to library!');
            } else if (result.error === 'Please log in to purchase games') {
                router.push('/login');
            } else {
                alert(result.error);
            }
        ...
        <div className={styles.container}>
            <div className={styles.contentWrapper}>
                    ...
                    <div className={styles.leftColumn}>
                        ...
                        {galleryList.length > 0 && (
                            <div className={styles.galleryThumbnails}>
                                ...
                                {galleryList.map((item: string, index: number) => (
                                    <div
                                        key={index}
                                        className={`${styles.thumbnail} ${currentMedia === item ? styles.active : ''}`}
                    ...
                    <div className={styles.rightColumn}>
                        <div className={styles.sidebar}>
                            ...
                            <div className={styles.ratingCard}>
                                ...
                                <img
                                    src={game.ageImage || "/ageimage/IARC_16.png"}
                                    alt="Rating"
                                    className={styles.ratingIcon}
                                />
                                <div className={styles.ratingInfo}>
                                    <span className={styles.ratingAge}>{game.ageRating || "16+"}</span>
                                    <span className={styles.ratingDesc}>{game.descRating || "Moderate Violence"}</span>
                                    <div className={styles.ratingDivider}></div>
                                    <span className={styles.ratingInteract}>{game.interactRating || "Users Interact"}</span>
                                </div>
                            </div>
                            ...
                            <div className={styles.primaryActions}>
                                <button
                                    ...
                                    onClick={inLibrary ? () => router.push('/library') : handlePurchase}
                                    disabled={isProcessing}
                                >
                                    {inLibrary ? "In Library" : (game.price === "Free" || game.currentPrice === "Free" ? "Get" : "Buy Now")}
                                </button>
                            ...
                            <button
                                ...
                                onClick={inWishlist ? () => router.push('/wishlist') : handleWishlistToggle}
                                disabled={isProcessing || inLibrary}
                            >
                                {inWishlist ? <FaBookmark size={14} /> : <FaRegBookmark size={14} />}
                ...
                <PaymentModal
                    ...
                    price={game.currentPrice || game.price || 'Free'}
                />
            </div>
        </div>
    );
